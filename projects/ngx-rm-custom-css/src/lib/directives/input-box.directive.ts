...
export class InputBoxDirective  implements OnChanges {
    ...
    onBeforeInput(e: InputEvent): any {
      if (isNaN(Number(e.data)) && this.numberOnly) {
        if (
    ...
    onKeyDown(e: KeyboardEvent): any {
      ...
      if(this.inputElement.value.length){
        const allowedLength =this.maxLength-(this.inputElement.value.length+1); 
        if(allowedLength<0)
        {
          e.preventDefault();
        }
    ...
    private sanitizeInput(input: string): string {
      ...
      if (this.maxLength > 0) {
        ...
        {
          const allowedLength =this.maxLength-input.length; 
          result = allowedLength > 0 ? input : input.substring(0, this.maxLength);
        }
      }
